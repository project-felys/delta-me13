...
def get_felysneko_match_sub(language: str) -> MatchSub:
    ...
    return MatchSub(
        nickname=nickname, is_male=False, slow_path=True, enable_warning=True
    )


def get_fast_felysneko_match_sub() -> MatchSub:
    return MatchSub(
        nickname="银河猫猫侠", is_male=False, slow_path=False, enable_warning=False
    )


def get_slow_path_only_match_sub() -> MatchSub:
    return MatchSub(
        nickname="{NICKNAME}", is_male=False, slow_path=True, enable_warning=False
    )
